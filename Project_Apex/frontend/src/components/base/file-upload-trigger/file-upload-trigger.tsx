...
export const FileTrigger = (props: FileTriggerProps) => {
    ...
    // Apply non-standard `capture` attribute at runtime on supporting devices to avoid static compat warnings.
    useEffect(() => {
        if (defaultCamera && inputRef.current) {
            try {
                inputRef.current.setAttribute("capture", defaultCamera);
            } catch {
                // no-op: attribute unsupported
            }
        }
    }, [defaultCamera]);
    ...
    return (
        <>
            ...
            <input
                ...
                // @ts-expect-error - webkitdirectory is not a standard HTML attribute but supported by browsers
