        ...
        >
            <div
                className={cx(
                    ...
                    // Use a CSS variable to avoid inline styles
                    "[transform:translateX(calc(-100%+var(--progress)))]",
                    progressClassName,
                )}
                data-progress={percentage}
                // Expose progress as CSS variable via inline style on a wrapper-friendly way using attribute selector in CSS if needed
            />
        </div>
    );
};
        ...
        case "top-floating":
            return (
                <div className="relative flex flex-col items-end gap-2">
                    {baseProgressBar}
                    <div
                        className="absolute -top-2 -translate-x-1/2 -translate-y-full rounded-lg bg-primary_alt px-3 py-2 shadow-lg ring-1 ring-secondary_alt [left:var(--progress)]"
                        data-progress={percentage + "%"}
                    >
        ...
        case "bottom-floating":
            return (
                <div className="relative flex flex-col items-end gap-2">
                    {baseProgressBar}
                    <div
                        className="absolute -bottom-2 -translate-x-1/2 translate-y-full rounded-lg bg-primary_alt px-3 py-2 shadow-lg ring-1 ring-secondary_alt [left:var(--progress)]"
                        data-progress={percentage + "%"}
                    >
