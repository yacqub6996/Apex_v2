...
export const NavItemBase = ({ current, type, badge, href, icon: Icon, children, truncate = true, onClick }: NavItemBaseProps) => {
    ...
    const badgeElement =
        badge && (typeof badge === "string" || typeof badge === "number") ? (
            <Chip label={badge} color="default" size="small" sx={{ ml: 1 }} />
        ) : (
            badge
        );
    ...
    const externalIcon = isExternal && <Share className="size-4 stroke-[2.5px] text-fg-quaternary" />;
    ...
    if (type === "collapsible") {
        return (
            <summary className={cx("px-3 py-2", styles.root, current && styles.rootSelected)} onClick={onClick}>
                ...
                <ExpandMore aria-hidden="true" className="ml-3 size-4 shrink-0 stroke-[2.5px] text-fg-quaternary in-open:-scale-y-100" />
            </summary>
        );
    }
