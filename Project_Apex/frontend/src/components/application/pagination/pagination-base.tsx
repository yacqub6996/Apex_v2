...
export interface PaginationRootProps {
    ...
    total: number;
    children: ReactNode;
    /** The style of the pagination root (deprecated, prefer className). */
    style?: CSSProperties;
...
const PaginationRoot = ({ total, siblingCount = 1, page, onPageChange, children, style: _style, className }: PaginationRootProps) => {
    ...
    return (
        <PaginationContext.Provider value={paginationContextValue}>
            <nav aria-label="Pagination Navigation" className={className}>
                <ul className="list-none p-0 m-0 flex gap-1">{children}</ul>
            </nav>
...
interface TriggerProps {
    ...
    /** The style of the trigger (deprecated, prefer className). */
    style?: CSSProperties;
...
const Trigger: FC<TriggerProps> = ({ children, style: _style, className, asChild = false, direction, ariaLabel }) => {
    ...
    if (asChild && isValidElement(children)) {
        return cloneElement(children, {
            ...
            // style removed to satisfy lint; rely on classes
            className: [computedClassName, (children.props as HTMLAttributes<HTMLElement>).className].filter(Boolean).join(" ") || undefined,
        } as HTMLAttributes<HTMLElement>);
    }

    return (
        <button aria-label={ariaLabel || defaultAriaLabel} onClick={handleClick} disabled={isDisabled} className={computedClassName}>
            {children}
        </button>
    );
};
...
export interface PaginationItemProps {
    ...
    /** The style object of the pagination item (deprecated, prefer className). */
    style?: CSSProperties;
...
const PaginationItem = ({ value, isCurrent, children, style: _style, className, ariaLabel, asChild = false }: PaginationItemProps) => {
    const context = useContext(PaginationContext);
    if (!context) {
        throw new Error("Pagination components must be used within a <Pagination.Root />");
    }
...
const PaginationEllipsis: FC<PaginationEllipsisProps> = ({ children, style: _style, className }) => {
    const computedClassName = typeof className === "function" ? className() : className;

    return (
        <li>
            <span className={computedClassName} aria-hidden="true">
                {children}
            </span>
        </li>
    );
