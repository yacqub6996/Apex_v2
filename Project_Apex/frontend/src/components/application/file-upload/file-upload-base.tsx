...
import { CheckCircle, Delete, CloudUpload } from "@mui/icons-material";
...
export const FileListItemProgressFill = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, className }: FileListItemProps) => {
    ...
    return (
        <motion.li layout="position" className={cx("relative flex gap-3 overflow-hidden rounded-xl bg-primary p-4", className)}>
            {/* Background progress (no inline styles) */}
            <div className="absolute inset-0 -z-10" aria-hidden="true">
                <ProgressBar labelPosition="bottom" max={100} min={0} value={progress} className={cx("h-full w-full", isComplete && "opacity-0")} />
            </div>
            {/* Hidden native progress for screen readers */}
            <progress className="sr-only" max={100} value={progress} aria-label={`Uploading ${name}`} />
            {/* Inner ring. */}
            <div
                className={cx(
            ...
            <div className="relative flex min-w-0 flex-1">
                            ...
                            {!failed && (
                                <>
                                    ...
                                    <div className="flex items-center gap-1">
                                        ...
                                        {!isComplete && <CloudUpload className="size-4 stroke-[2.5px] text-fg-quaternary" />}
                ...
                <ButtonUtility color="tertiary" tooltip="Delete" icon={Delete} size="xs" className="-mt-2 -mr-2 self-start" onClick={onDelete} />
            </div>
        </motion.li>
    );
};
