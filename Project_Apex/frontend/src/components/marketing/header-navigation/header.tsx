    ...
    return (
        <div className="flex flex-col gap-8 border-t border-secondary px-4 py-6">
                ...
                <ul className="grid grid-flow-col grid-cols-2 grid-rows-4 gap-x-6 gap-y-3">
                    {footerNavItems.map((navItem) => (
                        <li key={navItem.label}>
                            <Button size="large" href={navItem.href} variant="text">
                                {navItem.label}
                            </Button>
                        </li>
                    ))}
            ...
            <div className="flex flex-col gap-3">
                ...
                <Button size="large" href="/signup">Sign up</Button>
            ...
            <div className="flex size-full max-w-container flex-1 items-center pr-3 pl-4 md:px-8">
                ...
                >
                                        ...
                                        {navItem.menu ? (
                                            <AriaDialogTrigger>
                                                <AriaButton className="flex cursor-pointer items-center gap-0.5 rounded-lg px-1.5 py-1 text-md font-semibold text-secondary outline-focus-ring transition duration-100 ease-linear hover:text-secondary_hover focus-visible:outline-2 focus-visible:outline-offset-2">
                                                    ...
                                                    <ExpandMoreIcon className="size-4 rotate-0 text-fg-quaternary transition duration-100 ease-linear in-aria-expanded:-rotate-180" />
                                                </AriaButton>
                    ...
                    <div className="hidden items-center gap-3 md:flex">
                        {/* High-contrast, round, non-shrinking toggle on desktop header */}
                        <ThemeToggle appearance="contrast" />
                        {/* Auth buttons only on lg+ to preserve space on laptops */}
                        <div className="hidden lg:flex items-center gap-3">
                            <Button size={isFloating ? "medium" : "large"} href="/login" variant="contained">
                                Log in
                            </Button>
                            <Button size={isFloating ? "medium" : "large"} href="/signup" variant="contained">
                                Sign up
                            </Button>
                        </div>
                    </div>

                    {/* Mobile quick actions + menu (right-aligned) */}
                    <div className="md:hidden ml-auto flex items-center gap-2">
                        {/* Theme toggle visible on mobile header with contrast */}
                        <ThemeToggle color="tertiary" appearance="contrast" />
                        ...
                        <AriaDialogTrigger>
                            <AriaButton
                                ...
                                className={({ isFocusVisible, isHovered }) =>
                                    cx(
                                        "group cursor-pointer rounded-lg p-2",
                            ...
                            <AriaPopover
                                ...
                                className="h-calc(100%-72px) scrollbar-hide w-full overflow-y-auto shadow-lg"
                                offset={0}
                                crossOffset={20}
                                containerPadding={0}
